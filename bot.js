  ...
  client.on('message', async msg => {
    if (msg.isMentioned(client.user)) {
      ...
      msg.author.send({
        embed: {
          color: 3447003,
          title: 'Only Tunes Radio -> Commands',
          fields: [
            {
              name: PREFIX + 'radio',
    ...
    if (command === 'radio') {
      ...
      if (args[1] === undefined) {
        radio.playRadio(msg.member.voiceChannel, msg, 'Only Tunes', 'https://listen.openstream.co/5467/audio')
      }
    }
    ...
    if (command === 'list' || command === 'radiolist') {
      msg.channel.send({
        embed: {
          color: 3447003,
          title: 'Only Tunes Radio -> Radio list ',
          fields: [
            {
              name: PREFIX + 'Main',
              value: 'Radio: **Only Tunes**'
            }
          ],
          timestamp: new Date()
        }
    ...
    if (command === 'leave' || command === 'stop') {
      ...
      if (voiceChannel && voiceChannel.id === msg.guild.voiceConnection.channel.id) {
        console.log('Leaving a channel and stopped playing Only Tunes Radio')
